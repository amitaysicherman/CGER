...
def get_data(pooling, src_model, src_tokenizer, tgt_tokenizer, gen_mol, return_files=False, cold_smiles=0, cold_fasta=0,
    ...
    if train_encoder:
        pos_valid = SrcTgtDataset(src_valid, tgt_valid, src_tokenizer, tgt_tokenizer, None, pooling=pooling,
                                  train_encoder=True)
        pos_test = SrcTgtDataset(src_test, tgt_test, src_tokenizer, tgt_tokenizer, None, pooling=pooling,
                                 train_encoder=True)
    else:
    ...
    if train_encoder:
        neg_valid = SrcTgtDataset(src_neg_valid, tgt_neg_valid, src_tokenizer, tgt_tokenizer, None, pooling=pooling,
                                  train_encoder=True)
        neg_test = SrcTgtDataset(src_neg_test, tgt_neg_test, src_tokenizer, tgt_tokenizer, None, pooling=pooling,
                                 train_encoder=True)
    else:
        neg_valid = SrcTgtDataset(src_neg_valid, tgt_neg_valid, src_tokenizer, tgt_tokenizer, src_model,
                                  pooling=pooling)
